...
WMWindow.prototype.init = function() {
    ...
    this.element.find('.panel-body').contextMenu({
        menuObject : PANE_CONTEXT_MENU,
        menuSelected : function(invokedOn, selectedItem) {
            self.handlePaneContextMenu(invokedOn, selectedItem);
        }
    });
}
...
WMWindow.prototype.handlePaneContextMenu = function(invokedOn, selectedItem) {
    panelBody = $(invokedOn);
    switch (selectedItem.attr('id')) {
        case 'split-vertically':
            this.splitPaneVertically(panelBody.parent(), '50%');
            break;
        case 'split-horizontally':
            this.splitPaneHorizontally(panelBody.parent(), '50%');
            break;
        case 'close-pane':
            this.closePane(panelBody.parent());
            break;
        case 'open-tab':
...
WMWindow.prototype.splitPaneVertically = function(pane, percent) {
    pane_id = pane.attr('id');
    WindowManager.splitPaneVertically(this, pane_id, percent);
}

WMWindow.prototype.splitPaneHorizontally = function(pane, percent) {
    pane_id = pane.attr('id');
    WindowManager.splitPaneHorizontally(this, pane_id, percent);
}
