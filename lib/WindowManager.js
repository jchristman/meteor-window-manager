...
WM.prototype.getTabs = function(pane_id) {
    var uwp = this.getUserWindowProfile();
    var tabs;
...
WM.prototype.setPanePos = function(wm_window, parent_con_id, new_percentage) {
    ...
    this.setPanePosHelper(win, parent_con_id, new_percentage);
    this.updateWindow(win);
}
...
WM.prototype.splitPaneVertically = function(wm_window, pane_id, percentage) {
    var uwp = this.getUserWindowProfile();
    var win = this.getWindowById(uwp, wm_window.id());
    
    this.splitPaneHelper(win, pane_id, percentage, 'vertical');
    console.log(win);
    this.updateWindow(win);
}

WM.prototype.splitPaneHorizontally = function(wm_window, pane_id, percentage) {
    var uwp = this.getUserWindowProfile();
    var win = this.getWindowById(uwp, wm_window.id());
    
    this.splitPaneHelper(win, pane_id, percentage, 'horizontal');
    this.updateWindow(win);
}

WM.prototype.splitPaneHelper = function(pane_tree, pane_id, percentage, orientation) {
    ...
    if (pane_tree.panes == undefined) return false;
    if (this.splitPaneHelper(pane_tree.panes.pane1, pane_id, percentage, orientation)) return true;
    if (this.splitPaneHelper(pane_tree.panes.pane2, pane_id, percentage, orientation)) return true;
...
WM.prototype.closePane = function(wm_window, pane_id) {
    ...
    this.closePaneHelper(win, pane_id, undefined, wm_window);
