...
(function($){
  ...
  var resizerP = $('<div id=resizerproxy>').mousemove(function(e){
    if (resizeMask & 1) {
      ...
      resizerPos.height = Math.abs(resizerPos.top - resizerPos.bottom);
    }
    else if (resizeMask & 4) {
      resizerPos.bottom += (lastMouseY-e.pageY) * -1;
      resizerPos.height = Math.abs(resizerPos.top - resizerPos.bottom);
    }
    if (resizeMask & 8) {
      resizerPos.left += (lastMouseX-e.pageX) * -1;
      resizerPos.width = Math.abs(resizerPos.left - resizerPos.right);
    }
    else if (resizeMask & 2) {
      resizerPos.right += (lastMouseX-e.pageX) * -1;
      resizerPos.width = Math.abs(resizerPos.left - resizerPos.right);
    }
    lastMouseX = e.pageX;
    lastMouseY = e.pageY;
  ...
  .mouseup(function(){
    resizerP.hide();
    $win.css(translatePos(resizerPos)).WM('ensure_viewable');
    $win = undefined;
    return false;
  });
  ...
  var onStartResize = function(e){
    ...
    $win.addClass('focused').css('zIndex',++zIndex);
    minH = $win.find('> .windowtitlebar').height() + 5;
    resizerPos = getPos($win);
    resizerPos.right = resizerPos.left + $win.width();
    resizerPos.bottom = resizerPos.top + $win.height();
    resizerPos.width = $win.width() + 2;
    resizerPos.height = $win.height() + 2;
    lastMouseX = e.pageX;
    lastMouseY = e.pageY;
    resizerC.css(resizerPos);
    resizerP.show();
  ...
  .mouseup(function(){
    moverP.hide();
    $win.css(translatePos(moverPos)).WM('ensure_viewable').removeClass('moving');
    $win = undefined;
    return false;
  });
  ...
  var onStartMove = function(e){
    ...
    $win.WM('raise');
    moverPos = getPos($win);
    moverPos.width = $win.width()+2;
    moverPos.height = $win.height()+2;
  ...
  var getPos=function(w){
    ...
    var parentPos = w.parent().offset(); 
    pos.parentW = w.parent().width();
    pos.parentH = w.parent().height();
    pos.parentTop = parentPos.top;
    pos.parentLeft = parentPos.left;
    return pos;
  };
  ...
  methods.ensure_viewable=function(){
    this.filter('.window').each(function(){
      ...
      if (p.top + w.height() > p.parentH) // bottom bound
        w.css('top', p.parentH - w.height() - 1);
      if (p.left < 0)
        w.css('left',0);
      if (p.left + w.width() > p.parentW)
        w.css('left', p.parentW - w.width() - 1);
    });
    return this;
  };
  ...
  methods.minimize = function() {
    this.each(function(){
      ...
      w.css({left:'auto',top:'auto',right:0,width:300,height:minH,zIndex:10})
       .removeClass('focused')
       .addClass('minimized'); 
    });
    $().WM('retileMin');
  ...
  methods.maximize = function() {
    ...
    this.each(function(){
      ...
      savePos(w);
      // njh: changed top and bottom to make room for bars
      w.css({left:0,top:51,bottom:51,right:0,width:'auto',height:'auto'})
  ...
  methods.raise = function() {
    ...
    if(zIndex > 1000){
      zIndex = 10;
      $(".window").not(w).css('zIndex', zIndex-1);
    }
    return this;
  };
  ...
  // jrc : Changed to add to div
  methods.open = function(parent,lnk,target,opts) {
    if (! parent) parent=window.top.document.body;
    if (! opts) opts = {};
    var w = $(template);
  ...
  methods.register = function(w) {
    ...
    // set default window title
    var tbt = tb.children('.titlebartext').text('default');
    tbt[0].onselectstart = cancelBubble;
    tbt[0].unselectable = "on";
  ...
  $(document).delegate('a.jquery-wm','click',function(e){
    ...
    var t = $(this);
    window.top.jQuery().WM('open',window.top.document.body,this.href,this.target,
